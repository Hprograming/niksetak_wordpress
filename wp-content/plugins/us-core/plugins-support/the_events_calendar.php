...
if ( ! function_exists( 'the_events_calendar_us_grid_filter_main_query_args' ) ) {
	add_filter( 'us_grid_filter_main_query_args', 'the_events_calendar_us_grid_filter_main_query_args', 501, 1 );
	/**
	 ...
	 * @return array Returns a modified list of query arguments
	 */
	function the_events_calendar_us_grid_filter_main_query_args( $query_args ) {
		// For archive pages, return the result unchanged
		if ( is_archive() ) {
			return $query_args;
		}

		// Get current post ID
		$current_page_id = is_search()
			? (int) us_get_option( 'search_page', /* default */get_the_ID() )
			: get_the_ID();

		// Include events whose date has expired
		if ( $events_calendar_show_past = get_post_meta( $current_page_id, '_us_first_grid_events_calendar_show_past', /* single */TRUE ) ) {
			return (array) the_events_calendar_us_grid_listing_query_args( $query_args, get_defined_vars() );
		}
