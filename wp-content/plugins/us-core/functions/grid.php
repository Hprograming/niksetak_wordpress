...
if ( ! function_exists( 'us_define_content_and_apply_grid_filters' ) ) {
	...
	function us_define_content_and_apply_grid_filters( &$query_vars, $tax_query = NULL ) {
		...
		if ( ! $_us_content_id ) {
			...
			if ( ! $_us_content_id AND us_get_option( 'enable_page_blocks_for_sidebars', 0 ) ) {
...
if ( ! function_exists( 'us_save_post_first_grid_data' ) ) {
	...
	function us_save_post_first_grid_data( $post_id ) {
		/**
		 * Finds the first grid and saves the settings in the page's meta box
		 * Note: Meta key mask `_us_first_grid_{attr_key}`
		 *
		 * @param WP_Post $post
		 * @return string
		 */
		$func_search_first_grid = function ( $post ) {
			if (
				...
				// Get grid attributes
				$atts = shortcode_parse_atts( $matches[/* atts as string */1] );
				if ( ! is_array( $atts ) ) {
					$atts = array();
				}

				// Get or set the post type in the grid
				if ( empty( $atts['post_type'] ) ) {
					$atts['post_type'] = (array) us_config( 'elements/grid.params.post_type.std', /* default */'post' );
				}
				if ( is_array( $atts['post_type'] ) ) {
					$atts['post_type'] = array_unique( $atts['post_type'] );
				}

				// Save post types
				$post_meta = array( '_us_first_grid_post_type' => $atts['post_type'] );

				// Save events show past (The Events Calendar)
				if ( us_post_type_is_available( 'tribe_events', (array) $atts['post_type'] ) ) {
					$post_meta['_us_first_grid_events_calendar_show_past'] = isset( $atts['events_calendar_show_past'] );
				}

				// Save selected_taxonomies and products_include
				if ( us_post_type_is_available( $atts['post_type'], array_keys( us_grid_available_taxonomies() ) ) ) {
					$post_meta['_us_first_grid_products_include'] = us_arr_path( $atts, 'products_include' );
					$post_meta['_us_first_grid_selected_taxonomies'] = us_grid_get_selected_taxonomies( $atts );
				}
