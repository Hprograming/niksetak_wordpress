...
! function( $, undefined ) {
	...
	var USBuilderPreview = function() {
		...
		self.$document
			...
			.on( 'mouseup', $usbcore.debounce( self._events.endDrag, /* wait */2 ) ) // call after `_events.elmSelected`
			// Other events
			.on( 'mouseup', '[data-usbid]', $usbcore.debounce( self._events.elmSelected, /* wait */1 ) ) // call before `_events.endDrag`
	...
	$.extend( $usbPreviewPrototype, {
		...
		_getMutationObserver: function( id ) {
			...
			var observer = new MutationObserver( $usbcore.debounce( self.setHighlightsPosition.bind( self ), /* wait */1 ) );
		...
		_createHighlight: function( id ) {
			...
			var highlightElm = self.highlight.cloneNode( true ),
				...
				dataHighlight = $usbcore.toPlainObject( $usbcore.$attr( node, 'data-usb-highlight' ) );
	...
	$.extend( $usbPreviewPrototype, {
		...
		_$events: {
			...
			showPreloader: function( targetId, position, isContainer, id ) {
				...
				if (
					targetId === 'container'
					&& ! $usbcore.isUndefined( self._preloaders[ id || targetId ] )
				) {
					return;
				}
			...
			onPreviewParamChange: function( targetId, instructions, value, fieldType, isResponsiveValue ) {
				...
				$target // Send event on element change in us-builder
