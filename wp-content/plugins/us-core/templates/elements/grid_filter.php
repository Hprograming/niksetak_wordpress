...
if ( ! is_archive() ) {
	...
	// Get current post ID
	if ( is_search() ) {
		$current_page_id = (int) us_get_option( 'search_page', /* default */get_the_ID() );
	} else {
	...
	if ( ! $selected_taxonomies = get_post_meta( $current_page_id, $meta_key, TRUE ) ) {
		$page_block_ids = array();
		us_get_recursive_parse_page_block( get_post( $current_page_id ), function ( $page_block ) use ( &$page_block_ids ) {
	...
	if ( ! is_search() AND $post_types = (array) get_post_meta( $current_page_id, '_us_first_grid_post_type', /* single */TRUE ) ) {
		$query_args['post_type'] = (string) us_arr_path( $post_types, '0', /* default */'post' );
	}

	// Get products include of the first grid
	if ( $products_include = (string) get_post_meta( $current_page_id, '_us_first_grid_products_include', /* single */TRUE ) ) {
		// Show Sale products
		if (
	...
	unset( $current_page_id, $page_block_ids, $meta_key, $products_include );
}
...
$query_args = (array) apply_filters( 'us_grid_filter_main_query_args', $query_args );
	...
	if ( in_array( $ui_type, array( 'checkbox', 'radio' ) ) AND ! empty( $item_values ) ) {
		...
		foreach ( $item_values as $item_value ) {
			...
			if ( ! empty( $filters_args['taxonomies_query_args'][ $source ][ $item_value_slug ] ) ) {
				...
				$old_item_query_args = $item_query_args;
				us_apply_grid_filters( NULL, $item_query_args );
				if ( $item_query_args !== $old_item_query_args ) {
