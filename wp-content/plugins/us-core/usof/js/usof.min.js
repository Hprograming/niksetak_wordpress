...
if(self._params.editor!==!1){self._params.editor.codemirror.lint=!1;self.editor=wp.codeEditor.initialize(self.$input[0],self._params.editor||{});self.editorDoc=self.editor.codemirror.getDoc();self.setValue(self.$input.val());self.editor.codemirror.on('focus',self._events.editorFocused);self.editor.codemirror.on('blur',self._events.editorFocused)}}else{self.$input.on('keyup',function(){self.parentSetValue(self.getValue());self.setValue(self.$input.val())})}},setValue:function(value){var self=this;if(!!self._params&&self._params.hasOwnProperty('encoded')&&self._params.encoded){value=usof_rawurldecode(usof_base64_decode(value))}
if(self.editor!==_undefined&&wp.hasOwnProperty('codeEditor')){self.editorDoc.off('change',self._events.editorChange);self.editorDoc.setValue(value);self.editorDoc.on('change',self._events.editorChange);if(self.isEditLive()){setTimeout(function(){self.editorDoc.cm.refresh()},0.5)}}},getValue:function(){var self=this,value=self.editor!==_undefined&&wp.hasOwnProperty('codeEditor')?self.editorDoc.getValue():self.$input.val();if(self._params!==_undefined&&self._params.encoded){value=usof_base64_encode(usof_rawurlencode(value))}
